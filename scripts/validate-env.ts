...
const placeholderHostnamePattern =
  /(?:^|[._-])(?:example|test|xxx|placeholder|your|replace[-_]?me)(?:$|[._-])/i
...
const placeholderCredentialPrefixPattern =
  /^(?:your|replace[-_]?me|change[-_]?me|insert[-_]?|set[-_]?|put[-_]?|todo[-_]?|xxx[-_]?)/i
...
const isPlaceholderCredential = (value: string): boolean => {
  ...
  return (
    exactPlaceholderCredentials.has(normalized) ||
    placeholderCredentialPrefixPattern.test(normalized)
  )
}
...
const isPlaceholderUrl = (value: string): boolean => {
  try {
    ...
    return (
      ...
      hostname === '127.0.0.1' ||
      hostname === '0.0.0.0' ||
      placeholderHostnamePattern.test(hostname) ||
      hostname.includes('replaceme')
    )
  } catch {
    return false
  }
...
const validateUrl = (
  ...
  if (mode === 'production' && isPlaceholderUrl(value)) {
    ...
    mode === 'production' &&
    (isPlaceholderCredential(parsedUrl.username) ||
      isPlaceholderCredential(parsedUrl.password))
  ) {
    errors.push(
      `Invalid variable ${name}: placeholder credentials are not allowed in production`
    )
  }
}
...
export function validateEnvironment(
  ...
  for (const name of urlVariables) {
    ...
    if (isPresent(value)) {
      validateUrl(name, value, mode, errors)
    }
  }

  if (mode === 'production') {
    const urlVariableSet = new Set<string>(urlVariables)
    const credentialVariables = [...requiredProductionVariables, ...modelVariables].filter(
      (name) => !urlVariableSet.has(name)
    )

    for (const name of credentialVariables) {
      const value = env[name]
      if (isPresent(value)) {
        validateCredential(name, value, mode, errors)
      }
    }
  }
