/**
 * Rytr API Discovery Script (Standalone)
 * 
 * Discovers available use cases and their input context requirements
 * Run with: RYTR_API_KEY=your_key node --import tsx scripts/discover-rytr-use-cases-standalone.ts
 */
...
async function discoverRytrUseCases() {
  ...
  if (!RYTR_API_KEY) {
    throw new Error('RYTR_API_KEY environment variable is required');
  }
  ...
  try {
    ...
    const useCasesResponse = await fetch(`${RYTR_API_BASE}/use-cases`, {
      headers: {
        'Authentication': `Bearer ${RYTR_API_KEY}`,
