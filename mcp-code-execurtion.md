...
Codemode is an experimental pattern of using LLMs to generate executable code that performs tool calls, inspired by CodeAct. Instead of directly calling predefined tools, the LLM generates Python/JavaScript code that orchestrates multiple tool calls and complex logic.
...
How It Works
Tool Detection: When the LLM needs to use tools, instead of calling them directly, it generates a codemode tool call
Code Generation: The system generates executable JavaScript code that uses your tools
Safe Execution: Code runs in an isolated worker environment with controlled access to your tools
Result Return: The executed code's result is returned to the user
Demo Application
...
Install dependencies in the root (npm install)
Build dependencies in the root (npm run build)
Navigate to examples/codemode/
