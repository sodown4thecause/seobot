...
## Per-PR reconciliation decisions

This decision table is evaluated against `origin/main` at `8251e5e`. The exact title, branch, head/base SHA, counts, reviews, review dates, and evidence limitation remain in the ledger above; the full per-file inventories remain below. `Tests run` records what was independently run for this evidence task, not an inferred GitHub check result.

PR #82 exact title (encoding-corrected): `chore: repo cleanup - doc reorganization, asset moves, AGENTS.md reference files`.

| PR | Purpose | Changed areas | Mergeability | Overlap with default branch | Overlap with other open PRs | Unique valuable files | Stale / superseded files | Tests run | Recommended action |
|---|---|---|---|---|---|---|---|---|---|
| #83 | Rescue agent skills, planning material, CI, harness contracts, and profile-RAG work from an orphaned worktree. | 206 files: `.agents/skills/`, planning/docs, `.github/workflows/ci.yml`, harness, onboarding/profile RAG, vector search, tests. | Connector: `mergeable=false`; CI/check state unavailable. | Broad additions are mostly absent from main, but `lib/db/vector-search.ts` is a current-main modification target and must be reviewed against the live schema. | Skills/docs overlap #82 and #74; product/runtime and test areas overlap #81/#79/#78; profile-RAG/harness assets are largely unique. | `.github/workflows/ci.yml`; `docs/harness/team-spec.md`; `lib/chat/harness-contracts.ts`; `lib/onboarding/profile-rag.ts`; `lib/db/vector-search.ts` delta. | Bulk `.agents/skills/` rescue and planning material without an adoption decision; any files duplicating current docs or runtime conventions. | `pnpm install --frozen-lockfile --lockfile-only` passed on current checkout; no PR CI/check state was exposed; PR tests not rerun. | **Manually port** selected CI, harness, and profile-RAG files after tenant-isolation, migration, and workflow review. |
| #82 | Clean up/reorganize repository documentation and assets while carrying GEO, Workspace, artifact, and mode changes. | 177 files: docs/AGENTS references, archived migrations/content, dashboard/chat/workspace/artifacts, GEO routes and migrations, auth and deployment files. | Connector: `mergeable=false`; CI/check state unavailable. | Extensive overlap with merged #77/#80 and current main across modes, Workspace/artifacts, GEO, auth, docs, and deletions. | Major overlap with #76 on modes/GEO, #78 on cleanup/docs/artifact UI, and #81/#79 on chat/landing/observability; some GEO files overlap #83-era documentation. | `docs/architecture/geo-elmo-cloakbrowser-azure.md`; `drizzle/0007_geo_tracking.sql`; `drizzle/0008_elmo_brand_id.sql`; selected archive/reference moves. | Broad deletions, duplicated mode/workspace/artifact implementation, and repository-wide AGENTS/archive moves that are not safe as a bundle. | Current frozen-lockfile validation passed; no PR CI/check state was exposed; PR tests not rerun. | **Manually port** unique GEO migration/deployment documentation after schema and runtime review; **close superseded** bundle. |
| #81 | Post-#77 cleanup, pnpm migration, Magic UI landing redesign, and observability/usage polish. | 154 files across package tooling, landing/chat/dashboard, observability, analytics, Sentry, content deletions, and tests. | Connector: `mergeable=false`; CI/check state unavailable. | Based on pre-#80 product state and overlaps current main in chat, landing, agents, analytics, Sentry/Langfuse, package files, and deletions. | Substantially duplicates #79; also overlaps #78 UI/chat and #76 mode surfaces. | `app/api/webhooks/langfuse/route.ts`; `components/providers/analytics-provider.tsx`; `components/providers/posthog-identify.tsx`; selected observability additions if still needed. | Package-manager migration bundle, deleted content/features, and broad landing/chat rewrites already represented or superseded by #77/#80/current main. | Current frozen-lockfile validation passed; no PR CI/check state was exposed; PR tests not rerun. | **Close superseded** branch; **manually port** only current-baseline-compatible observability fixes. |
| #79 | Complete the same pnpm/Magic UI/landing/chat polish plan with usage, analytics, and cleanup changes. | 160 files across the same landing/chat/agent/observability surface as #81, plus deletions and tests. | Connector: `mergeable=false`; CI/check state unavailable. | Broad stale-base overlap with current main, including package files, routing, chat, landing, analytics, observability, and deleted legacy features. | Near-duplicate of #81; overlaps #78 UI/chat and #76 mode changes. | `components/chat/message-bubble.tsx`; `components/dashboard/usage-summary-card.tsx`; selected `lib/analytics/` or observability deltas after review. | Duplicate #81 implementation, deleted content/features, old package migration, and broad rewrites. | Current frozen-lockfile validation passed; no PR CI/check state was exposed; PR tests not rerun. | **Close superseded** branch; **manually port** any uniquely useful, current-compatible file. |
| #78 | Add Cursor-style UI/UX fixes, AGENTS architecture docs, cleanup, and chatbot error/artifact improvements. | 99 files: UI/auth/chat/artifact changes, AGENTS docs, migration/content archive moves, tool error UI, and deletions. | Connector: `mergeable=false`; CI/check state unavailable. | Overlaps current main in auth, chat, artifacts, landing, sidebar, docs, and migrations; several moves/deletions are repository-wide churn. | Overlaps #82 cleanup/archive, #81/#79 chat/landing/artifacts, and #76 mode UI. | `components/chat/tool-ui/tool-error-card.tsx`; selected artifact/tool UI changes; current architecture docs if still accurate. | Archived migration/content moves, deleted `GEMINI.md`/`design.png`/legacy hooks, and UI changes already superseded by current main. | Current frozen-lockfile validation passed; no PR CI/check state was exposed; PR tests not rerun. | **Manually port** selected tool-error/UI fixes; **close superseded** bundle. |
| #76 | Align SEO, GEO/AEO, and Content modes across landing, dashboard, routing, copy, and deployment. | 32 files: mode routing/context/selectors, landing/dashboard copy, GEO docs/deployment, auth/config, and focused tests. | Connector: `mergeable=false`; CI/check state unavailable. | Core intent is already in merged #77/#80 and current main; direct merge would reintroduce the older base. | Overlaps #82 on mode/GEO/workspace, #78/#81/#79 on UI surfaces. | None required from the implementation bundle; `docs/specs/platform-modes.md` and GEO deployment docs are reference candidates only if current docs lack them. | Mode implementation, product copy, auth/config, and tests superseded by #77/#80/current main. | Current frozen-lockfile validation passed; no PR CI/check state was exposed; PR tests not rerun. | **Close superseded** branch; retain reference only. |
| #75 | Prevent signed-in users from being bounced from the dashboard to Polar checkout and adjust admin allowlisting. | 3 files: dashboard layout, auth admin helper, and a subscription-guard test. | Connector: `mergeable=false`; CI/check state unavailable. | Small but stale overlap with current dashboard/auth/billing code; compare behavior against current Better Auth and Polar flow. | Limited overlap with #82 auth/admin changes and #76 auth changes. | `tests/unit/dashboard-subscription-guard.test.ts` as behavioral evidence; possible narrow `app/dashboard/layout.tsx` or `lib/auth/admin.ts` fix. | Any redirect/admin change already present in current main after #72/#80; do not port blindly from the stale base. | Current frozen-lockfile validation passed; no PR CI/check state was exposed; PR tests not rerun. | **Cherry-pick selected commits** only after current auth/billing behavior review. |
| #74 | Document Cursor Cloud development-environment setup. | One file: `AGENTS.md`. | Connector: `mergeable=false`; CI/check state unavailable. | Documentation-only overlap with current root instructions and the AGENTS docs added by #82/#83. | Overlaps #82/#83 documentation and skill/reference additions. | Any still-correct, non-secret Cursor Cloud setup notes. | Environment instructions that conflict with current package manager, auth, database, or deployment conventions. | Current frozen-lockfile validation passed; no PR CI/check state was exposed; PR tests not rerun. | **Manually port** verified documentation; **close superseded** PR after porting. |
...
Captured on 2026-07-12 in `codex/production-readiness-pr-reconciliation` at
commit `18f41c5` before this evidence update. No production credentials,
production Neon database, VPS, CI, or E2E environment was used.

| Command | Result | Evidence / limitation |
|---|---|---|
| `node --version` | Passed | `v24.14.0` satisfies the package Node `>=22` contract. |
| `pnpm --version` | Passed | `11.5.0`, matching `packageManager`. |
...
This baseline is not a production-readiness approval. CI/E2E, production smoke,
provider contracts, browser exploration, Autonoma, VPS, backup/restore, and
branch-protection validation remain deferred.

## Reconciliation draft PR

The reviewed branch is published as draft [PR #84](https://github.com/sodown4thecause/seobot/pull/84), targeting `main`. The draft head is
`fa3e1779196aea02e894e0ba11475f7e7a4f1721`; it is open, unmerged, and not a
