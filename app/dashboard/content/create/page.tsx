...
function CreateContentForm() {
  ...
  useEffect(() => {
    ...
    const getProfile = async () => {
      // TODO: Fetch business profile from API using Drizzle
      // Tables needed: business_profiles
      // Need to add API route: /api/business-profile
      
      // For now, just set a mock ID
      setBusinessProfileId('mock-profile-id')
    }
    getProfile()
  }, [user, userLoaded])
  
  const handleKeywordAnalysis = async () => {
    if (!keyword || !businessProfileId || !user) return
  ...
  const handleGenerateContent = async () => {
    if (!keyword || !businessProfileId || !contentData || !user) return
            ...
            <div className="flex items-start space-x-4 mb-6">
              ...
              <div>
                ...
                <p className="text-gray-600">
                  Enter a keyword or topic you want to write about, and I&apos;ll analyze your opportunity.
                </p>
              </div>
            </div>
