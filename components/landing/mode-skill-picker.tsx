...
export function ModeSkillPicker() {
  ...
  return (
    <div className="mx-auto w-full max-w-3xl">
      <div
        className={`relative rounded-2xl border ${accent.borderPanel} ${accent.bgPanel} bg-black/60 backdrop-blur-sm transition-colors`}
      >
        {/* Top bar: active mode + deep link */}
        <div className="flex items-center justify-between border-b border-white/10 px-4 py-3">
          <div className="flex items-center gap-2">
            <span className={`h-2 w-2 rounded-full ${accent.selectorDot}`} />
            <span className={`font-mono text-[11px] uppercase tracking-[0.18em] ${accent.textLabel}`}>
          ...
          <div className="flex items-end gap-3">
            ...
            <div className="relative">
              <button
                ...
                className={`flex h-10 w-10 items-center justify-center rounded-lg border border-white/15 bg-white/[0.03] text-zinc-300 transition-colors hover:bg-white/[0.06] ${accent.promptHoverBorder}`}
              >
                <Plus
              ...
              <AnimatePresence>
                {menuOpen && (
                  <motion.div
                    ...
                    className="absolute bottom-12 left-0 z-30 w-64 rounded-xl border border-white/15 bg-black/95 p-2 shadow-[0_24px_60px_rgba(0,0,0,0.6)] backdrop-blur"
                  >
                    <p className="px-2 py-1 font-mono text-[10px] uppercase tracking-[0.18em] text-zinc-600">
                      Modes
                    </p>
                    {CHAT_MODE_LIST.map((m) => {
                      ...
                      return (
                        <button
                          ...
                          className={`flex w-full items-start gap-3 rounded-lg border border-transparent px-2 py-2 text-left transition-colors hover:bg-white/[0.04] ${mAccent.promptHoverBorder} ${isActive ? 'bg-white/[0.04]' : ''}`}
                        >
            ...
            <div className="flex-1">
              <textarea
                ...
                className="w-full resize-none rounded-lg border border-white/10 bg-black/40 px-3 py-2 text-sm text-zinc-200 placeholder:text-zinc-600 focus:border-white/30 focus:outline-none"
              />
            </div>
